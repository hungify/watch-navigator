...
class HuaweiWearEngineServiceTest {
    ...
    @Test
    fun checkPermissions_whenPermissionGranted_returnsTrue() = runTest(testDispatcher) {
    ...
    @Test
    fun sendNavMessage_whenSendFailsWithErrorCode_returnsFailureResult() = runTest(testDispatcher) {
        val permTask = mockSuccessfulTask(true)
        every { authClient.checkPermission(Permission.DEVICE_MANAGER) } returns permTask
        val devTask = mockSuccessfulTask(true)
        every { deviceClient.hasAvailableDevices() } returns devTask
    ...
    @Test
    fun sendNavMessage_whenSendFails_updatesConnectionStateToDisconnected() = runTest(testDispatcher) {
        val permTask = mockSuccessfulTask(true)
        every { authClient.checkPermission(Permission.DEVICE_MANAGER) } returns permTask
        val devTask = mockSuccessfulTask(true)
        every { deviceClient.hasAvailableDevices() } returns devTask
    ...
    @Test
    fun sendNavMessage_serializesConcurrentSendsPreservingOrder() = runTest(testDispatcher) {
        val permTask = mockSuccessfulTask(true)
        every { authClient.checkPermission(Permission.DEVICE_MANAGER) } returns permTask
        val devTask = mockSuccessfulTask(true)
        every { deviceClient.hasAvailableDevices() } returns devTask
    ...
    @Test
    fun pingWatch_whenWatchAppRunning_returnsSuccessTrue() = runTest(testDispatcher) {
        val permTask = mockSuccessfulTask(true)
        every { authClient.checkPermission(Permission.DEVICE_MANAGER) } returns permTask
        val devTask = mockSuccessfulTask(true)
        every { deviceClient.hasAvailableDevices() } returns devTask
    ...
    @Test
    fun pingWatch_whenWatchAppNotRunning_returnsSuccessFalse() = runTest(testDispatcher) {
        val permTask = mockSuccessfulTask(true)
        every { authClient.checkPermission(Permission.DEVICE_MANAGER) } returns permTask
        val devTask = mockSuccessfulTask(true)
        every { deviceClient.hasAvailableDevices() } returns devTask
    ...
    @Test
    fun startAutoReconnect_retriesWithExponentialBackoffAndResumesOnConnection() = runTest(testDispatcher) {
        val permTask = mockSuccessfulTask(true)
        every { authClient.checkPermission(Permission.DEVICE_MANAGER) } returns permTask
        val devTask = mockSuccessfulTask(true)
        every { deviceClient.hasAvailableDevices() } returns devTask
    ...
    @Test
    fun stopAutoReconnect_cancelsRetryLoopImmediately() = runTest(testDispatcher) {
        val permTask = mockSuccessfulTask(true)
        every { authClient.checkPermission(Permission.DEVICE_MANAGER) } returns permTask
        val devTask = mockSuccessfulTask(true)
        every { deviceClient.hasAvailableDevices() } returns devTask
    ...
    @Test
    fun release_cancelsAutoReconnectAndResetsState() = runTest(testDispatcher) {
        service.startAutoReconnect()
        assertThat(service.isReconnecting.value).isTrue()
    ...
    @Test
    fun startAutoReconnect_capsDelayAtMaxRetryDelayMs() = runTest(testDispatcher) {
        val permTask = mockSuccessfulTask(true)
        every { authClient.checkPermission(Permission.DEVICE_MANAGER) } returns permTask
        val devTask = mockSuccessfulTask(true)
        every { deviceClient.hasAvailableDevices() } returns devTask
    ...
    @Test
    fun startAutoReconnect_allowsRestartFromWithinOnReconnectedCallback() = runTest(testDispatcher) {
        val permTask = mockSuccessfulTask(true)
        every { authClient.checkPermission(Permission.DEVICE_MANAGER) } returns permTask
        val devTask = mockSuccessfulTask(true)
        every { deviceClient.hasAvailableDevices() } returns devTask
    ...
    @Test
    fun checkConnection_whenTimeoutOccurs_returnsErrorStateWithoutCancellingCaller() = runTest(testDispatcher) {
        val permTask = mockSuccessfulTask(true)
        every { authClient.checkPermission(Permission.DEVICE_MANAGER) } returns permTask
    ...
    @Test
    fun startAutoReconnect_whenCheckConnectionTimesOut_continuesRetryLoop() = runTest(testDispatcher) {
        val permTask = mockSuccessfulTask(true)
        every { authClient.checkPermission(Permission.DEVICE_MANAGER) } returns permTask
    ...
    @Test
    fun stopAutoReconnect_cancelsActiveCallbackJob() = runTest(testDispatcher) {
        val permTask = mockSuccessfulTask(true)
        every { authClient.checkPermission(Permission.DEVICE_MANAGER) } returns permTask
        val devTask = mockSuccessfulTask(true)
        every { deviceClient.hasAvailableDevices() } returns devTask
    ...
    @Test
    fun startAutoReconnect_whenPermissionUnauthorized_haltsRetryLoop() = runTest(testDispatcher) {
