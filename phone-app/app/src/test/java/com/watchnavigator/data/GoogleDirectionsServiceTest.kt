...
class GoogleDirectionsServiceTest {
    ...
    fun buildDirectionsRequest_withProxyUrlAndToken_constructsValidProxyRequest() {
        ...
        val request =
            service.buildDirectionsRequest(
    ...
    fun buildDirectionsRequest_withTrailingSlashOrExplicitEndpoint_formatsUrlCleanly() {
        ...
        val serviceExplicitWithTrailingSlash =
            GoogleDirectionsService(
                serverUrl = "https://watch-navigator-proxy.example.workers.dev/api/v1/directions/",
                serverToken = "token"
            )
        val req3 = serviceExplicitWithTrailingSlash.buildDirectionsRequest("0,0", "1,1", TravelMode.WALKING)
        assertThat(req3.url.encodedPath).isEqualTo("/api/v1/directions")

        val serviceDirectionsWithTrailingSlash =
            GoogleDirectionsService(
                serverUrl = "https://watch-navigator-proxy.example.workers.dev/directions/",
                serverToken = "token"
            )
        val req4 = serviceDirectionsWithTrailingSlash.buildDirectionsRequest("0,0", "1,1", TravelMode.WALKING)
        assertThat(req4.url.encodedPath).isEqualTo("/api/v1/directions")
    }
    ...
    fun buildDirectionsRequest_withHttpProxyUrl_throwsIllegalArgumentException() {
        val service =
            GoogleDirectionsService(
                serverUrl = "http://insecure-proxy.example.workers.dev",
    ...
    @Test
    fun getDirections_successfulProxyResponse_returnsParsedNavRoute() =
        runBlocking {
            val mockJson =
                """
                {
    ...
    @Test
    fun getDirections_proxyReturns401Unauthorized_returnsHelpfulErrorMessage() =
        runBlocking {
            val errorJson =
                """
                {
    ...
    @Test
    fun getDirections_proxyReturns500ServerError_returnsHelpfulErrorMessage() =
        runBlocking {
            val errorJson =
                """
                {
    ...
    @Test
    fun getDirections_networkFailure_returnsFailureResult() =
        runBlocking {
            val mockClient =
                OkHttpClient
                    .Builder()
    ...
    @Test
    fun getDirections_socketTimeout_returnsConsistentTimeoutErrorMessage() =
        runBlocking {
            val mockClient =
                OkHttpClient
                    .Builder()
            ...
            assertThat(result.isFailure).isTrue()
            assertThat(result.exceptionOrNull()?.message).isEqualTo("The directions request timed out. Check your connection and try again.")
        }

    @Test
    fun getDirections_withHttpProxyUrl_returnsFailureResult() =
        runBlocking {
            val service =
                GoogleDirectionsService(
                    serverUrl = "http://insecure-proxy.example.workers.dev",
            ...
            assertThat(result.exceptionOrNull()?.message).contains("Invalid NAV_SERVER_URL: http://insecure-proxy.example.workers.dev")
        }

    @Test
    fun getDirections_proxyReturnsRedirectToHttp_failsWithoutCleartextDowngrade() =
        runBlocking {
            val mockClient =
                OkHttpClient
                    .Builder()
                    .addInterceptor { chain ->
                        Response
                            ...
                            .code(302)
                            .message("Found")
                            .header("Location", "http://insecure-downgrade.example.com/directions")
            ...
            val origin = LatLng(21.0, 105.0)
            val destination = LatLng(21.1, 105.1)
            val result = service.getDirections(origin, destination)

            assertThat(result.isFailure).isTrue()
            val exception = result.exceptionOrNull()
            assertThat(exception).isInstanceOf(IOException::class.java)
            assertThat(exception?.message).contains("302")
        }
}
