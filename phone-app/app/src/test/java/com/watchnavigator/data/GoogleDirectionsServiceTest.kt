...
class GoogleDirectionsServiceTest {
    ...
    fun buildDirectionsRequest_withTrailingSlashOrExplicitEndpoint_formatsUrlCleanly() {
        ...
        val serviceExplicitWithTrailingSlash = GoogleDirectionsService(
            serverUrl = "https://watch-navigator-proxy.example.workers.dev/api/v1/directions/",
            serverToken = "token"
        )
        ...
        val serviceDirectionsWithTrailingSlash = GoogleDirectionsService(
    ...
    @Test
    fun getDirections_socketTimeout_returnsConsistentTimeoutErrorMessage() = runBlocking {
        val mockClient = OkHttpClient.Builder()
            .addInterceptor {
    ...
    @Test
    fun getDirections_withHttpProxyUrl_returnsFailureResult() = runBlocking {
        val service = GoogleDirectionsService(
            serverUrl = "http://insecure-proxy.example.workers.dev",
            serverToken = "secret-token"
        )
    ...
    @Test
    fun getDirections_proxyReturnsRedirectToHttp_failsWithoutCleartextDowngrade() = runBlocking {
        val mockClient = OkHttpClient.Builder()
            .addInterceptor { chain ->
                Response.Builder()
                    .request(chain.request())
